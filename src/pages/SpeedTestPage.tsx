...
export default function SpeedTestPage({
  ...
  return (
    <div>
      <div className="page-title">连通性测速</div>
      <div className="page-desc">
        向各密钥配置的端点发起轻量请求，测量连通性与延迟。
        测速是唯一的联网操作，仅在你点击「开始测速」时发生。
      </div>
      ...
      {results.length === 0 ? (
        ...
        <div className="card" style={{ padding: 0, overflow: "hidden" }}>
          ...
          >
            结果：成功 <span className="speed-ok">{okCount}</span> /{" "}
            {results.length} 条
          </div>
      ...
      >
        安全说明：测速请求由 Rust 后端直连你配置的端点发出，已禁用自动重定向
        （防止密钥被转发到第三方）；应用自身不含任何遥测、更新检查或第三方连接。
      </div>
    </div>
  );
}
