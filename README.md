...
加密保险库存储你的所有模型 API Key，支持一键测速、明文/加密导出、一键写入环境变量与 coding 工具配置。应用自身**无遥测、无自动更新、无第三方连接**——仅在你主动点击「测速」或「检查更新」提供商信息时才联网。
...
| 密钥分类 | 按量计费 / Coding Plan 两类管理；表单与列表均有标识，按量 key 跑 coding agent 有开销提示 |
| Coding 工具一键导入 | Coding Plan 密钥一键写入 Claude Code（环境变量）、Codex CLI（config.toml）、Kimi Code（config.toml）配置，写前自动备份、幂等替换；Cursor 提供复制指引 |
| 控制台 | 8 家主流提供商的按量付费 / Coding Plan 购买与管理入口，三栏表直达后台 |
| 提供商信息更新 | 设置页一键从仓库拉取最新提供商模板（名称 / Base URL 等），只下载不上传 |
...
| 安装体验 | macOS dmg 紧凑布局（app 居左、Applications 居右）；关窗即退出进程，重开需重新解锁 |
...
1. **最小联网**：应用不包含任何遥测、崩溃上报、自动更新检查或第三方 SDK；仅两个联网路径，均需你主动点击——「测速」（直连你配置的端点）与设置页「检查更新」提供商信息（只下载仓库 providers.json，不上传任何数据）。
...
6. **生物识别解锁（可选）**：开启后主密码托管于操作系统安全存储——macOS 为 Keychain 通用密码项（仅本应用可读），Windows 为 Credential Locker；读取前先经系统身份验证（Touch ID，可回退登录密码 / Windows Hello 指纹·面容·PIN），应用自身不落盘主密码。解锁期间主密码以会话形式驻留 Rust 内存（Zeroizing），关窗即退出进程、会话随之清除，重开需重新解锁；Linux 不提供该入口。
