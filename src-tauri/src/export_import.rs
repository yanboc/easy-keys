...
mod tests {
    ...
    fn test_encrypted_export_import_roundtrip() {
        ...
        let out = encrypted_export(&records, "export-pass-123").unwrap();
        assert!(out.contains("EASYKEYS-EKEY"));
        // 密文不应含明文密钥
        assert!(!out.contains("sk-secret-123"));

        let back = encrypted_import(&out, "export-pass-123").unwrap();
        assert_eq!(back.len(), 1);
        assert_eq!(back[0].api_key, "sk-secret-123");
    }
    ...
    fn test_encrypted_import_wrong_password() {
        let records = vec![sample()];
        let out = encrypted_export(&records, "export-pass-123").unwrap();
        let err = encrypted_import(&out, "wrong-pass").unwrap_err();
        assert!(err.message.contains("口令错误"));
    }

    #[test]
    fn test_plain_json_roundtrip() {
        let records = vec![sample()];
        let json = plain_json_export(&records).unwrap();
        let back = plain_json_import(&json).unwrap();
        assert_eq!(back.len(), 1);
        assert_eq!(back[0].name, "k");
    }

    #[test]
    fn test_plain_json_invalid() {
        let err = plain_json_import("not json at all").unwrap_err();
